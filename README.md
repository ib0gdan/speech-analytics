...
транскрибируется, размечается на **Оператора** и **Клиента**, после чего один звонок
разбирают **четыре LLM-агента** (классификация, качество, compliance, резюме), а серию звонков —
**пятый агент трендов**. Результат доступен и в чате **OpenWebUI**, и через REST
(`POST /analyze` для одного звонка, `POST /analyze-batch` для нескольких).
...
- [Обоснование решений](#обоснование-решений)
  - [Pipe, а не Filter / Tools / Actions](#2-pipe-а-не-filter--tools--actions)
...
Поднимутся пять сервисов:

| Сервис | Порт | Назначение |
|---|---|---|
| `openwebui` | 3000 | Веб-чат |
| `pipelines` | 9099 | OpenWebUI Pipeline (внешний сервер) |
| `api` | 8000 | REST `POST /analyze`, `POST /analyze-batch` |
| `files` | 8899 | Раздаёт `test_data/` для демонстрации |
| `proxy` | 8080 | Единая точка входа для туннеля: чат + REST + `/grafana/` |

> Prometheus и Grafana (дашборд метрик) — опциональны, поднимаются профилем `metrics`:
> `docker compose --profile metrics up -d`. Обычный запуск их не включает.
...
### 2. Pipe, а не Filter / Tools / Actions

OpenWebUI даёт несколько точек расширения, и выбор между ними — это выбор того, чем система
является для пользователя.

- **Pipe** — наш выбор. Pipe регистрируется в чате как **отдельная «модель»**: пользователь
  выбирает «MTBank Call Analytics» и шлёт сообщение. Для нас анализ звонка и есть та «модель»,
  которая порождает ответ, — ровно то, что описывает Pipe. Точка входа — метод `pipe()` в
  `pipelines/mtbank_pipeline.py`.
- **Filter** (`inlet`/`outlet`) — не та форма. Filter пред- или постобрабатывает ответ **другой**
  модели (чистит ввод, дополняет вывод). Мы ничей ответ не оборачиваем — мы сами его порождаем.
- **Tools** (function calling) — не та форма и рискованно по бюджету. Tool срабатывает, когда LLM
  **сама решит** его вызвать. Наш анализ детерминирован входом: есть аудио-ссылка → анализируем,
  без «догадается ли модель дёрнуть функцию». Вешать тяжёлый CPU-bound ASR на усмотрение модели
  значит терять и предсказуемость, и лимит 60 секунд.
- **Actions** — это кнопки под сообщением (экспорт, «сделать TTS»); ортогонально нашей задаче.
- **Valves** — используем поверх Pipe. Класс `Valves` в `pipelines/mtbank_pipeline.py` выносит
  `LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`, `WHISPER_MODEL`, `WHISPER_COMPUTE_TYPE` в настройки
  OpenWebUI — их правят из UI без пересборки образа. Отсюда и «сменить провайдера/модель одной
  переменной».

### 3. Groq как LLM-бэкенд
...
### 4. `faster-whisper small`, а не `medium` (с оговоркой)
...
### 6. Собственный Supervisor, а не LangGraph
...
### 7. `quality_score.total` считает код, а не модель
