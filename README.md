...
```bash
docker compose exec api pytest                  # 65 тестов, ~5 с
...
| `tests/test_metrics.py` | 4 | Инкремент счётчиков в `run_analysis`/батче, `/metrics`-эндпоинт |
| **Итого** | **65** | |
...
распределение), **топ тематик** — плюс доля нарушений комплаенса и p95 длительности анализа.
